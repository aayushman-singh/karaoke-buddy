...
def _card(
    ...
    x, y = xy
    draw.rounded_rectangle((x, y, x + 210, y + 205), radius=6, fill="#f8f8f8", outline="#cfcfcf")
...
def _home_image(path: Path) -> None:
    ...
    draw.rounded_rectangle((24, 92, 442, 148), radius=5, fill="#ffffff", outline="#b8b8b8")
    draw.rounded_rectangle((458, 92, 876, 148), radius=5, fill="#ffffff", outline="#b8b8b8")
    draw.text((162, 111), "Open a video file", fill="#111111", font=_font("segoeui.ttf", 18))
    draw.text((594, 111), "Paste YouTube link", fill="#111111", font=_font("segoeui.ttf", 18))
    draw.text((30, 184), "Recent videos", fill="#111111", font=_font("segoeuib.ttf", 18))
...
def _slider(
    ...
    draw.text((360, track_y + 20), status, fill="#aaaaaa", font=_font("segoeui.ttf", 14))
...
def _player_image(path: Path) -> None:
    ...
    draw.text((330, 124), "Video preview", fill="#d8d8d8", font=_font("segoeuib.ttf", 32))
    ...
    _slider(draw, 532, "Silence the singer", "Off", "Full", 570, "Guide vocals: 35% audible")
    draw.rounded_rectangle((56, 590, 228, 626), radius=5, fill="#f7f7f7")
    draw.text((84, 599), "Save this version", fill="#111111", font=_font("segoeui.ttf", 14))
    draw.text((738, 599), "Back to library", fill="#aaaaaa", font=_font("segoeui.ttf", 14))
    image.save(path)
